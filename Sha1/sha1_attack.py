...
def calculate_mac_length_extension():
    ...
    plt.plot(range(len(new_hash_values)), new_hash_values, marker='o', linestyle='-', color='b')
    plt.title('Hash Values Over Time - Length Extension Attack')
    plt.xlabel('Time')
...
# Calculate Button (for length extension attack) with time series plot
#attack_button_with_plot = tk.Button(root, text="Perform MAC Length Extension Attack with Plot", command=calculate_mac_length_extension_and_plot)
#attack_button_with_plot.pack()
